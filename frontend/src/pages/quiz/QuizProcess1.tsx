...
import {
  startRecording,
  stopRecording,
} from "../../redux/store/reducers/quizProctoringSlice";
...
export default function QuizProcess1(_: IQuizProcess1Props) {
  ...
  const { proctoringRecordState } = useAppSelector(
    (state) => state.quizProctoring
  );
  ...
  const [showQuizSubmitPreviewModal, setShowQuizSubmitPreviewModal] =
    React.useState<boolean>(false);
  const [quizSubmitPreviewPayload, setQuizSubmitPreviewPayload] =
    React.useState<IMarkedObj>({});
  ...
  React.useEffect(() => {
    ...
    if (!quizConfig.lazy && questions.length === 0) {
      ExamService.fetchQuestions(quiz_id)
        .then((response) => {
        ...
        <div className="container mx-auto pt-5" data-aos="fade-up">
          <div className="flex flex-col md:flex-row">
            ...
            <div className="justify-end">
              ...
              {quizConfig.paginted && (
                <Pagination
                  showOnlyPrimitive={false}
                  page={currentQuestionPage}
                  rowsPerPage={1}
                  count={questions.length}
                  onChangePage={handleQuestionPageChange}
                />
              )}
